...
const { WebVideoCtrl, $, ActiveXObject, mWebVideoCtrl } = window;
$(function () {
  // 检查插件是否已经安装过
...
function getChannelInfo() {
  ...
  //     async: false,
  //     success: function (xmlDoc) {
  //       alert(`${szDeviceIdentify} 获取模拟通道成功！`);
  ...
  //     },
  //     error: function (status, xmlDoc) {
  //       console.log("I_GetAnalogChannelInfo error", status, xmlDoc);
  //       alert(`${szDeviceIdentify} 获取模拟通道失败！`);
  ...
  WebVideoCtrl.I_GetDigitalChannelInfo(szDeviceIdentify, {
    ...
    error: function (status, xmlDoc) {
      console.log("I_GetDigitalChannelInfo error", status, xmlDoc);
...
function getDevicePort() {
  ...
  oPort = WebVideoCtrl.I_GetDevicePort(szDeviceIdentify);
...
export function clickStartRealPlay(iStreamType) {
  ...
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    szDeviceIdentify = ipVal,
    iRtspPort = parseInt(oPort.iRtspPort, 10),
    iChannelID = parseInt(oSel[0]?.id, 10),
